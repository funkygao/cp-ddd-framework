...
public abstract class BaseRouter<Ext extends IDomainExtension, Identity extends IIdentity> implements IDomainService {
     ...
     *
     * @param identity 业务身份
     * @param reducer  收敛逻辑 如果为空，则等同于{@link #firstExtension(Identity)}
     * @param <R>      扩展点方法的返回值类型
     * @return null if not found
     */
    protected <R> Ext forEachExtension(@NonNull Identity identity, IReducer<R> reducer) {
     ...
     * @return null if not found
     */
    protected <R> Ext forEachExtension(@NonNull Identity identity, int timeoutInMs, IReducer<R> reducer) {
    ...
    private <Ext extends IDomainExtension, R> Ext findExtension(@NonNull Class<Ext> extClazz, @NonNull Identity identity, IReducer<R> reducer, Ext defaultExt, int timeoutInMs) {
        ExtensionInvocationHandler<Ext, R> proxy = new ExtensionInvocationHandler<>(extClazz, identity, reducer, defaultExt, timeoutInMs);
